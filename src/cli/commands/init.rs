...
pub async fn execute(name: Option<String>, path: Option<String>) -> Result<()> {
    ...
    if project_path.join("hatch.yaml").exists() || project_path.join("hatch.json").exists() {
        println!("⚠️  Manifest file already exists. Use --force to overwrite.");
        return Ok(());
    }

    // Create hatch.yaml manifest (default to YAML format)
    manifest_manager.create_manifest(&project_name, ManifestFormat::Yaml)?;
    ...
    println!();
    println!("Next steps:");
    println!("  1. Edit hatch.yaml to configure dependencies");
